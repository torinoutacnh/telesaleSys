...
namespace XProject.WebApi.Controllers
{
    ...
    public class CloudFoneController : ControllerBase
    {
        ...
        [Route("ClickToCallV1")]
        [HttpPost]
        public string CallV1(string prefix,string ext,string PhoneNumber)
        {
            ...
            var response = client.PostAsync("https://api.cloudfone.vn/api/CloudFone/AutoCall", content);
        ...
        [Route("GetCalls")]
        [HttpPost]
        public async Task<IActionResult> GetCalls(string start = "", string end = "", string typeget = "0", string callnum = "", string receivenum = "", string key = "", int pageindex = 1, int pagesize = 200)
        {
            if (start == "")
            {
                start = DateTime.Now.ToString("MM-dd-yyyy");
            ...
            string request = $"\"ServiceName\":CF - PBX0001442,\"AuthUser\":ODS000953,\"AuthKey\":d8e8990b - 60c6 - 4b50 - 9fac - f766078a3f19,\"TypeGet\":{typeget},\"DateStart\":{start},\"DateEnd\":{end},\"CallNum\":{callnum},\"ReceiveNum\":{receivenum},\"key\":{key},\"PageIndex\":{pageindex},\"PageSize\":{pagesize}";
