...
function GetReportAmountByDate() {
    ...
    $.ajax({
        ...
        url: "/Report/Dashboard/GetCallByDate",
        success: function (data) {
            var json = jQuery.parseJSON(data);
            var morrisDonutData = [{
                label: "Trả lời",
                value: json.totalCallAnswer
            }, {
                label: "Không trả lời",
                    value: json.totalCallNoAnswer
            }];
            charpie(morrisDonutData);
            var listOfObjects = [];
            ...
            $("#totalCall_M_W").text(ToAmount(json.totalAmount + "") + " Cuộc gọi");
            var html_trt =
                "<tr>" +
                "<th>" + "STT" + "</th>" +
                "<th>" + " Ngày " + "</th>" +
            ...
            document.getElementById("callDateTotal").innerHTML = ToAmount(json.totalAmount + "") + " Cuộc gọi";
            document.getElementById("callDateTotalAnswer").innerHTML = ToAmount(json.totalCallAnswer + "") + " Cuộc gọi";
            document.getElementById("callDateTotalNoAnswer").innerHTML = ToAmount(json.totalCallNoAnswer + "") + " Cuộc gọi";
            // khao sat
            //document.getElementById("callReview").innerHTML = ToAmount(Math.floor(json.totalAmount / 2) + "") + " Cuộc gọi";
          //  document.getElementById("callReviewBuyProduct").innerHTML = ToAmount(Math.floor(json.totalAmount / 2) + "") + " Cuộc gọi";
            ...
            for (x in listAmountDate) {
                var detail_html = "<tr>" +
                    ...
                    "<td> " + ToAmount(listAmountDate[x].Answer + "") + "</td>" +
                    "<td> " + ToAmount(listAmountDate[x].NoAnswer + "") + "</td>" +
            ...
            //call report
            //var listObjectCall = [];
            //for (x in json.date_call) {
            //    var obj_call = [];
            //    obj_call.push(json.date_call[x].Name);
            //    obj_call.push(json.date_call[x].Value);
            //    listObjectCall.push(obj_call);
            //}
            //// var dataset = [{ data: json.ChartWeeks, color: "#5482FF" }];
            //var dataChart_Call = [{
            //    data: listObjectCall,
            //    color: "#734ba9"
            //}];
            //drawMonthTotalCall("#totalCallDay", dataChart_Call);
            hideLoading();
        }
    });
}
...
/*
	...
	*/
function charpie(morrisDonutData) {
    ...
    for (x in list) {
        ...
        if (this.value == "Amount Agent") {
            detail_html = "<tr>" +
                ...
                "<td> " + ToAmount(list[x].Answer + "") + "</td>" +
                "<td> " + ToAmount(list[x].NoAnswer + "") + "</td>" +
